...
import bookingDetail from '../../testData/booking.json'


test.describe('API Testing - CRUD', () => {
    const baseURL = 'https://restful-booker.herokuapp.com/'
    test('Should get the booking details', async ({ request }) => {
        const response = await request.get('https://restful-booker.herokuapp.com/booking/15', {
            headers: {
    ...
    test('Create new booking', async ({ request }) => {
        ...
        console.log(resBody)
        // expect(resBody.bookingid).toBe(1720)
    ...
    test.skip('To update booking details', async ({ request }) => {
        const response = await request.put('https://restful-booker.herokuapp.com/booking/15', {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'Cookie': 'token=2bdb909e5f92d69',
        ...
        console.log(response.statusText())
        // const resBody = await response.json()
        // expect(resBody.firstname).toBe('Test')
        // expect(resBody.lastname).toBe('User')


    })

    test('To update user', async ({ request }) => {
